...
function renderAyahs(sura) {
  ...
  sura.ayahs.map(function (ayah, n) {
    output += `<li class="list-group-item bg-light"><bdo dir="rtl" class="py-3 d-block"><span class="ml-3 mb-2 sura-ayah badge badge-secondary">${suraNumber}:${
      ayah.numberInSurah
    }</span> ${n == 0 ? replaceBismillah(ayah.text) : ayah.text}</bdo> </li>`;
  });
    ...
    $.ajax({
      ...
      success: function (response) {
        ...
        response.data.ayahs.map(function (ayah, i) {
          ...
          if ("en.transliteration" == translationEngBnReTr) {
            $("ol.ayahs > li:nth-child(" + i + ") > bdo").after(
              suraTranslatedText
            );
          } else {
...
$("#query-quran").on("submit", function (e) {
  ...
  var sid = para.suraid;
  var totalAayahInSura = $(
    "#suraNameSelect option:nth-child(" + sid + ")"
  ).attr("ayahs");
...
function showSingleAyah(suraid, ayahno) {
  ...
  $.ajax({
    url: "https://api.alquran.cloud/v1/ayah/" + suraid + ":" + ayahno,
    method: "GET",
    crossDomain: true,
    dataType: "json",
    success: function (response) {
      // show single ayah
      $("ol.ayahs").append(
        `<li class="list-group-item bg-light"><bdo dir="rtl" class="py-3 d-block"><span class="ml-3 mb-2 sura-ayah badge badge-secondary">${
          response.data.surah.number
        }:${response.data.numberInSurah}</span> ${replaceBismillah(
          response.data.text
        )}</bdo> </li>`
      );

      // show ayah related sura name
      $(".jumbotron .sura-name").html(
        `<span class="badge badge-info">${response.data.surah.number}</span> ${response.data.surah.name}  ${response.data.surah.englishName} (${response.data.surah.englishNameTranslation})`
      );

      $(".jumbotron .sura-meta").html(
        `Total Ayahas: <b>${response.data.surah.numberOfAyahs}</b> | Revelation Type: <b>${response.data.surah.revelationType}</b> | Juz: <b>${response.data.juz}</b> | Ruku: <b>${response.data.ruku}</b> | Sajda: <b>${response.data.sajda}</b>`
      );

      // add new button group and buttons for single ayah
      $(".jumbotron").after(`<div class="btn-group-single-ayah"></div>`);
      $(".btn-group-single-ayah").append(
        `<button type="button" class="btn btn-outline-info btn-singl-ayah-eng" onclick="singleAyahEngBnReTr(${response.data.number}, 'en.sahih')">English</button>`
      );
      $(".btn-group-single-ayah").append(
        `<button type="button" class="btn btn-outline-success btn-singl-ayah-bn" onclick="singleAyahEngBnReTr(${response.data.number}, 'bn.bengali')">Bangla</button>`
      );
      $(".btn-group-single-ayah").append(
        `<button type="button" class="btn btn-outline-primary btn-singl-ayah-recite" onclick="singleAyahEngBnReTr(${response.data.number}, 'ar.alafasy')">Recitation</button>`
      );
      $(".btn-group-single-ayah").append(
        `<button type="button" class="btn btn-outline-danger btn-singl-ayah-transliteration" onclick="singleAyahEngBnReTr(${response.data.number}, 'en.transliteration')">Transliteration</button>`
      );
...
function singleAyahEngBnReTr(ayahno, engBnReTr) {
  ...
  $.ajax({
    url: "https://api.alquran.cloud/v1/ayah/" + ayahno + "/" + engBnReTr,
    method: "GET",
    crossDomain: true,
    dataType: "json",
    success: function (response) {
...
$(".search-word-form").on("submit", function (e) {
  ...
  $.ajax({
    url: "https://api.alquran.cloud/v1/search/" + word + "/all/en.sahih",
    method: "GET",
    crossDomain: true,
    dataType: "json",
    success: function (response) {
      if (undefined == response) {
        $(".quran-loader").hide();
        $(".jumbotron .sura-name").html(
          `<span class='text-danger'>Something wrong. Try another english word or correct spelling.</span>`
        );
        $(".jumbotron .sura-meta").hide();
        return;
      } else {
...
function renderSearchWordResult(data) {
  ...
  $(".jumbotron .sura-meta")
    .html(
      `Total <b class="text-info">${data.count}</b> words found from the noble Quran (Sahih International).`
    )
    .show();
